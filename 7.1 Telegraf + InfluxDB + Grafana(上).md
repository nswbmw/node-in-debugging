...
输入用户名 root 和密码 root 登录，进入初始化配置页，点击 Add data source 绿色按钮，如下填写：

![](./assets/7.1.4.png)

点击 Save & Test 保存配置。目前配置好了 Grafana 默认的 datasource 是 InfluxDB，接下来我们创建测试代码，产生测试数据。
...
回到 Grafana，点击左上角 Grafana 图标的下拉菜单，点击 Dashboards 回到仪表盘页继续完成配置，点击 New dashboard 绿色按钮，然后点击 Graph 类型的图表，创建了一个空的图表，如下所示：
...
- 请求量：包含了正常请求（200）和错误请求（4xx、5xx 等等）tps 的折线图
